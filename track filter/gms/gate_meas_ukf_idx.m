function valid_idx= gate_meas_ukf_idx(z,gamma,model,m,P,alpha,kappa,beta)
...
for j=1:plength
        ...
        Z_pred= gen_observation_fn_unline( model, X_ukf(1:model.x_dim,:), X_ukf((model.x_dim+1):(model.x_dim+model.w_dim),:) );
        ...
        nu= z- repmat(gen_observation_fn_unline(model,m(:,j),zeros(size(model.D,2),1)),[1 zlength]);
