...
P_update = zeros(model.x_dim,model.x_dim,plength,zlength);

for idxp=1:plength
    for idxz = 1 : zlength
        [qz_temp,m_temp,P_temp] = ukf_update_single(z(:, idxz),model,m(:,idxp),P(:,:,idxp),alpha,kappa,beta);
        qz_update(idxp,idxz) = qz_temp;
        m_update(:,idxp,idxz) = m_temp;
        P_update(:,:,idxp,idxz) = P_temp;
    end
end
