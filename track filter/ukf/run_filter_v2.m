function est = run_filter_v2(model,meas)
...
%   pages = {108532}}
%
% This 'v2' implementation shares similar structure to the robust multi-sensor
...
filter.L_max= 100;                  %limit on number of Gaussians in each track - CPHD filter
filter.elim_threshold= 1e-5;        %pruning threshold for Gaussians in each track - CPHD filter
filter.merge_threshold= 4;          %merging threshold for Gaussians in each track - CPHD filter
...
cphd.w_update(1)= eps;
cphd.m_update(:,1)= [0.1;0;0.1;0;0;0];
...
for k=2:meas.K
    ...
    [meas_ru_w, glmb_update, cphd] = jointpredictupdate(glmb_update,tt_birth,model,filter,meas,k,cphd);      
...
function [meas_ru_w , glmb_nextupdate, cphd_nextupdate] = jointpredictupdate(glmb_update,tt_birth,model,filter,meas,k,cphd)
...
for tabsidx=1:length(glmb_update.tt)
    ...
    tt_survive{tabsidx}.st(1) = ( ( beta_mean * (1-beta_mean)/ beta_var) - 1 ) * beta_mean;
    tt_survive{tabsidx}.st(2) = ( ( beta_mean * (1-beta_mean)/ beta_var - 1) ) * (1 - beta_mean);
end


%% Run CPHD filtering to estimate the clutter rate
% compute the PHD from the GLMB 
[phd, w_in, w_birth, cdn_in] = cphd_predict(glmb_update,  tt_survive, tt_birth, model, filter) ; 
[cphd_nextupdate, qz_update, m_update, P_update] = cphd_update(cphd, cdn_in, model, filter, phd, meas, w_in, w_birth, k);  
N_c = max(cphd_nextupdate.Nc_update, eps) ; 
...
for tabidx= 1:length(glmb_predict.tt)
    ...
    avpd(tabidx) = glmb_predict.tt{tabidx}.st(1)/ ... 
        (glmb_predict.tt{tabidx}.st(1) + glmb_predict.tt{tabidx}.st(2));
...
for tabidx= 1:length(glmb_predict.tt)
    for emm= glmb_predict.tt{tabidx}.gatemeas
        ...
        tt_update{stoidx}.st = glmb_predict.tt{tabidx}.st ; 
        tt_update{stoidx}.st(1) = tt_update{stoidx}.st(1) + 1 ; 
