...
filter_max_number = 3;                            % the number of fitlers
T=1; 
win_len= 10;
cutoff = 100;
   ...
   meas = gen_meas(model,truth);
    ...
    for filter_n= 1:filter_max_number 
        ...
        elseif filter_n == 2
            ...
            est4 = est;
        elseif filter_n == 3
            est =run_filter_v2(model,meas); % Robust-GLMB
        end
    ...
    for p_i = 1:filter_n
%         p_i=3;
%         plot_data(p_i).d1=save_data(p_i).d1;
%         plot_data(p_i).n=save_data(p_i).n;
%         for i = 1:length(win_len)
%         plot_data(p_i).d2{i}=save_data(p_i).d2{i};
%         end
...
%==========================================================
%                    plot figures(MC_max_runs)
%==========================================================

for i=1:filter_max_number
%     i=4;MC_max_runs=1/20;
...
legend('True number','AMDP-J-GLMB','DP-GLMB','Robust-GLMB','new-GLMB')
...
legend('AMDP-J-GLMB','DP-GLMB','Robust-GLMB','new-GLMB')
...
xlabel('Time (s)','interpreter','latex'),ylabel('Time cost')
legend('AMDP-J-GLMB','DP-GLMB','Robust-GLMB','new-GLMB')

% average performance per scan per run
c = {'AMDP-J-GLMB','DP-GLMB','Robust-GLMB','new-GLMB'};
for i=1:filter_max_number
    the_OSPA(i) = sum(plot_data(i).d1(:,1))/size(plot_data(i).d1,2);
    disp([c{i},' : ','     OSPA is ',num2str(the_OSPA(i),4)])
end

  a=plot_data(3).n ./truth.N'/50;
  b=plot_data(2).n ./truth.N'/50;
  sum=0;
for i=1:100
    sum=sum+b(i);
end