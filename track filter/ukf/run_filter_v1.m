...
cbmember.x_init= [0.1;0;0.1;0;0];
cbmember.P_init= diag([100 10 100 10 1]).^2;
...
for midx = 1 : length(tt_birth)
    ...
    tt_birth{midx}.u_b_tg=90;
    tt_birth{midx}.v_b_tg=10;
end
...
for k=2:meas.K
    ...
    cphd = est_pD(tt_birth , cphd, model, filter , meas , k) ; 
    %robust CBMEMBER filtering
    cbmember =jcbmember_est_clutter(tt_birth , cbmember, model, filter , meas , k) ; 
%    [meas_ru_w , glmb_update] = jointpredictupdate(glmb_update,tt_birth,model,filter,meas,k,cbmember.L(k),cbmember.pD{k});   
   [meas_ru_w , glmb_update] = jointpredictupdate(glmb_update,tt_birth,model,filter,meas,k,cbmember.L(k),cphd.P_D);   
